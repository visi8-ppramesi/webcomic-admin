...
const updateGenre = (id, data) => {
  return genreResource.update(id, data);
};
const createGenre = (data) => {
  return genreResource.store(data);
};

const defaultGenreForm = {
  is_draft: true,
  name: '',
  source_uri: '',
  id: undefined,
...
export default {
  ...
  data() {
    ...
    // };
    return {
      postForm: Object.assign({}, defaultGenreForm),
      loading: false,
      rules: {
  ...
  created() {
    if (this.isEdit) {
      ...
      this.postForm = Object.assign({}, defaultGenreForm);
    }
  ...
  methods: {
    ...
    submitForm() {
      if (this.isEdit){
        updateGenre(this.postForm.id, this.postForm)
          .then((response) => {
